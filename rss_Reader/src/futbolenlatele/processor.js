...
function formatJsonToCorrectMDAcceptedCharts(notice){
    ...
    notice.content='<div class="tags">'+matchListToContentCardFormat(notice)+'</div>';
    notice.content=notice.content.replaceAll(" : ", ",").replaceAll(": ", ",").replaceAll("&nbsp;","");
    //No notice.image
}


function matchListToContentCardFormat(notice){
    var htmlContent = '<div class="center_content">';
    notice.partidos.forEach((partido) => {
        let matchRaw= partido.equipo1+" vs "+partido.equipo2 + "<br>";
        htmlContent=htmlContent+matchRaw;
    });
